    ...
    try:
        ...
        while True:
            ...
            if data.get("type") == "handshake":
                ...
                # Pre-warm conversation when user connects
            ...
            # Persist user message to CRM.conversations (fire-and-forget)
            try:
                lead_id = data.get("lead_id")  # Optional lead_id for CRM context
                asyncio.create_task(save_user_message(conversation_id, message, lead_id=lead_id))
            except Exception as e:
                # Non-fatal: log error, continue processing
                logger.error(f"Failed to save user message: {e}")

            # Create a root span per user message
            tracer = None
