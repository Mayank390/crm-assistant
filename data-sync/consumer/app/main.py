...
def main() -> None:
    ...
    topic_pattern = get_env("KAFKA_TOPIC", "crm\..*")  # Updated for CRM
    group_id = get_env("KAFKA_GROUP_ID", "qdrant-consumer")
    qdrant_url = get_env("QDRANT_URL", "http://qdrant:6333")
    collection = get_env("QDRANT_COLLECTION", "crm")  # Updated for CRM
    ...
    while True:
        try:
            ...
            logger.error(f"Error in main consumer loop: {exc}", exc_info=True)
            time.sleep(5)
            try:
                consumer.close()
            except Exception:
